...
$validFiles = "*.{[Mm][Pp]3,[Oo][Gg][Gg],[Ww][Aa][Vv],[Mm][Pp]4,[Mm][Oo][Vv]}";
...
<div id="currentPlaying"><a>Frecuencias Sanadoras</a></div>
...
<video controls autoplay id="player">Tu navegador no soporta audio.</video>
<script>
...
$('body').on('click','.playListLink',function(){
  ...
  const filePath = encodeURI($(this).data('path') + '/' + fileName);
  player.src = filePath;

  // Opcional: Detectar extensión para ajustar estilos del reproductor
  const extension = fileName.split('.').pop().toLowerCase();
  if(['mp4', 'mov'].includes(extension)) {
    // Ajustar estilo del video, por ejemplo mostrarlo con un tamaño adecuado
    $('#player').show();
    $('#player').attr('controls', true);
  } else {
    // En caso de ser audio, se puede seguir usando el mismo elemento <video>
    // pero quizá ocultar la ventana de video si se desea
    $('#player').hide(); // Si prefieres no mostrar pantalla negra para audios
  }
